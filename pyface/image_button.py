...
class ImageButton ( Widget ):
    ...
    def __init__ ( self, parent, **traits ):
        ...
        if self.orientation == 'horizontal':
            ...
            dx       = idx + tdx + spacing
            dy       = max( idy, tdy )
            self._iy = hp2 + (old_div((dy - idy), 2))
            self._ty = hp2 + (old_div((dy - tdy), 2))
        else:
            self._iy = hp2
            ...
            dx       = max( idx, tdx )
            dy       = idy + tdy + spacing
            self._ix = wp2 + (old_div((dx - idx), 2))
            self._tx = wp2 + (old_div((dx - tdx), 2))
...
#-- wx event handlers ----------------------------------------------------------
    ...
    def _on_paint ( self, event ):
        ...
        ox       = old_div((wdx - self._dx), 2)
        oy       = old_div((wdy - self._dy), 2)
