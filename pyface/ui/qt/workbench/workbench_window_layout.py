...
class WorkbenchWindowLayout(MWorkbenchWindowLayout):
    ...
    _qt_editor_area = Instance(SplitTabWidget)
    ...
    def activate_editor(self, editor):
        if editor.control is not None:
            editor.control.show()
            self._qt_editor_area.setCurrentWidget(editor.control)
            editor.set_focus()
    ...
    def add_editor(self, editor, title):
        ...
        try:
            self._qt_editor_area.addTab(
                self._qt_get_editor_control(editor), title
            )

            if editor._loading_on_open:
                self._qt_editor_tab_spinner(editor, "", True)
        except Exception:
    ...
    def add_view(self, view, position=None, relative_to=None, size=(-1, -1)):
        ...
        try:
            self._qt_add_view(view, position, relative_to, size)
            view.visible = True
        except Exception:
    ...
    def close(self):
        ...
        self._qt_editor_area.editor_has_focus.disconnect(
            self._qt_editor_focus
        )

        self._qt_editor_area.clear()

        # Delete all dock widgets.
        for v in self.window.views:
            if self.contains_view(v):
                self._qt_delete_view_dock_widget(v)

    def create_initial_layout(self, parent):
        self._qt_editor_area = editor_area = SplitTabWidget(parent)

        editor_area.editor_has_focus.connect(self._qt_editor_focus)
        ...
        # needed.
        editor_area.focus_changed.connect(self._qt_view_focus_changed)

        editor_area.tabTextChanged.connect(self._qt_editor_title_changed)
        editor_area.new_window_request.connect(self._qt_new_window_request)
        editor_area.tab_close_request.connect(self._qt_tab_close_request)
        editor_area.tab_window_changed.connect(self._qt_tab_window_changed)

        return editor_area

    def contains_view(self, view):
        return hasattr(view, "_qt_dock")

    def hide_editor_area(self):
        self._qt_editor_area.hide()
    ...
    def reset_editors(self):
        self._qt_editor_area.setCurrentIndex(0)
        ...
        for v in self.window.views:
            ...
            for vid in view_ids:
                if vid == v.id:
                    ...
                    self._qt_create_view_dock_widget(v).setVisible(False)
    ...
    def get_editor_memento(self):
        ...
        editor_layout = self._qt_editor_area.saveState()
    ...
    def set_editor_memento(self, memento):
        ...
        def resolve_id(id):
            ...
            return self._qt_get_editor_control(editor)

        self._qt_editor_area.restoreState(editor_layout, resolve_id)
    ...
    def is_editor_area_visible(self):
        return self._qt_editor_area.isVisible()

    # ------------------------------------------------------------------------
    # Private interface.
    # ------------------------------------------------------------------------

    def _qt_editor_focus(self, new):
    ...
    def _qt_editor_title_changed(self, control, title):
        """ Handle the title being changed """
        for editor in self.window.editors:
            if editor.control == control:
                editor.name = str(title)

    def _qt_editor_tab_spinner(self, event):
        editor = event.object

        # Do we need to do this verification?
        tw, tidx = self._qt_editor_area._tab_widget(editor.control)
        ...
        if not event.new and not editor == self.window.active_editor:
            self._qt_editor_area.setTabTextColor(
                editor.control, QtCore.Qt.GlobalColor.red
            )

    @observe("window:active_editor")
    def _qt_active_editor_changed(self, event):
        """ Handle change of active editor """
        # Reset tab title to foreground color
        editor = event.new
        if editor is not None:
            self._qt_editor_area.setTabTextColor(editor.control)

    def _qt_view_focus_changed(self, old, new):
    ...
    def _qt_new_window_request(self, pos, control):
        """ Handle a tab tear-out request from the splitter widget. """

        editor = self._qt_remove_editor_with_control(control)
    ...
    def _qt_tab_close_request(self, control):
    ...
    def _qt_tab_window_changed(self, control):
        ...
        for window in self.window.workbench.windows:
            if window.control == control:
                ...
                window.editor_manager.add_editor(editor, kind)
                window.layout._qt_get_editor_control(editor)
                window.activate_editor(editor)
                editor.window = window
                break

    def _qt_remove_editor_with_control(self, control):
        ...
        for editor in self.window.editors:
            if editor.control == control:
                ...
                control.removeEventFilter(self._qt_mon)
    ...
    def _qt_get_editor_control(self, editor):
        ...
        if editor.control is None:
            ...
            editor.observe(self._qt_editor_tab_spinner, "_loading")

            self.editor_opened = editor

        def on_name_changed(event):
            editor = event.object
            self._qt_editor_area.setWidgetTitle(editor.control, editor.name)

        editor.observe(on_name_changed, "name")

        self._qt_monitor(editor.control)

        return editor.control

    def _qt_add_view(self, view, position, relative_to, size):
        """ Add a view. """
        ...
        dw = self._qt_create_view_dock_widget(view, size)
        mw = self.window.control

        try:
            rel_dw = relative_to._qt_dock
        except AttributeError:
            rel_dw = None
    ...
    def _qt_create_view_dock_widget(self, view, size=(-1, -1)):
        ...
        try:
            ...
            dw.toggleViewAction().toggled.connect(
                self._qt_handle_dock_visibility
            )
            dw.visibilityChanged.connect(self._qt_handle_dock_visibility)

            # Save the dock window.
            view._qt_dock = dw

            def on_name_changed(event):
                view._qt_dock.setWindowTitle(view.name)
    ...
    def _qt_delete_view_dock_widget(self, view):
    ...
    def _qt_handle_dock_visibility(self, checked):
    ...
    def _qt_monitor(self, control):
        ...
        try:
            ...
            mon = self._qt_mon = _Monitor(self)
