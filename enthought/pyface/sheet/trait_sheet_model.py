...
#------------------------------------------------------------------------------
""" A TraitSheetModel builds a sheet from a list of traits objects. Each row
...
# local imports
from sheet_model import SheetColumn, SheetModel, SheetSortEvent
from trait_sheet_cell_adapter import TraitSheetCellAdapter

# The classes below are part of the table specification.
class TraitSheetColumn(SheetColumn):
    """ Structure for holding column specifications in a TraitSheetModel. """
...
class TraitSheetSelection(HasTraits):
...
# The meat.
class TraitSheetModel(SheetModel):
    """ A TraitSheetModel builds a sheet from a list of traits objects. Each row
    ...
    # A 2-dimensional list/array containing the sheet data.
    data = List()#HasTraits)
    
    # The column definitions
    columns = Trait(None, None, List(Trait(None, Str, TraitSheetColumn)))
    ...
    def __init__(self, **traits):
        """ Create a TraitSheetModel object. """

        # Base class constructor
        super(TraitSheetModel, self).__init__(**traits)
        ...
        if self.columns is None or len(self.columns) == 0:
            if self.data is not None and len(self.data) > 0:
                ...
                for name, trait in self.data[0].traits().items():
                    if trait.type != 'event':
                        self._auto_columns.append(TraitSheetColumn(name = name))
            else:
                self._auto_columns = []
    ...
    def get_column_name(self, index):
        ...
        try:
            ...
            if isinstance(col, TraitSheetColumn):
    ...
    def get_cols_selection_value(self, cols):
        """ Returns a list of TraitSheetSelection objects containing the
        object corresponding to the sheet rows and the traits corresponding
        to the specified columns. """

        values = []
        for obj in self.data:
            for col in cols:
                values.append(TraitSheetSelection(obj = obj,
    ...
    def sort_by_column(self, col, reverse=False):
        ...
        try:
            ...
            sorter = cmp
            if isinstance(column, TraitSheetColumn) and \
        ...
        # now fire an event to tell the sheet we're sorted
        print 'firing sort event'
        self.column_sorted = SheetSortEvent(index = col, reversed = reverse)
    ...
    def get_rows_selection_value(self, rows):
        ...
        for row_index in rows:
            values.append(TraitSheetSelection(obj = self.data[row_index]))
    ...
    def get_cell_editor(self, row, col):
        ...
        # print 'TraitSheetModel.get_cell_editor row: ', row, ' col: ', col
        ...
        return TraitSheetCellAdapter(factory, obj, trait_name, '')
    ...
    def get_cell_selection_value(self, row, col):
        """ Returns a TraitSheetSelection object specifying the data stored
        in the table at (row, col). """

        obj = self.data[row]
        trait_name = self.__get_column_name(col)

        return TraitSheetSelection(obj = obj, trait_name = trait_name)

    def resolve_selection(self, selection_list):
        """ Returns a list of (row, col) sheet-cell coordinates that
        ...
        selected. For the TraitSheetModel, the objects in objlist must
        be TraitSheetSelection objects. """
    ...
    #########################################################################
    # protected 'SheetModel' interface.
    #########################################################################
    ...
    def _set_value(self, row, col, value):
        ...
        #print 'TraitSheetModel._set_value: new: ', value
    ...
    def _get_data_from_row(self, row, column):
        ...
        if row is not None and column is not None:
            if not isinstance(column, TraitSheetColumn):
    ...
    def _set_data_on_row(self, row, column, value):
        ...
        if row is not None and column is not None:
            if not isinstance(column, TraitSheetColumn):
                if hasattr(row, column):
                    # sometimes the underlying sheet gives us 0/1 instead
    ...
    def _on_row_name_trait_changed(self, new):
        """ Force the sheet to refresh when any underlying trait changes. """
    ...
    def _on_data_changed(self, object, name, old, new):
        """ Force the sheet to refresh when the underlying list changes. """
    ...
    def __get_column_name(self, col):
        ...
        if isinstance(column, TraitSheetColumn):
            name = column.name
